...
from mmm.cache import saturation_curve
...
setup_page(
    "Saturation Lab",
    "🌱",
    "The first dollars can have a large effect. As spend rises, each additional dollar may produce less incremental response.",
)
...
spend, median, interval = saturation_curve(
    ...
    max_spend,
    180,
    float(half) if active else None,
...
fig.add_trace(
    go.Scatter(
        ...
        marker=dict(size=14, color=PALETTE[2]),
        name="Observed median",
    )
)
fig.update_layout(
    title=f"{show} · {channel} response curve",
    xaxis_title="Weekly spend",
    yaxis_title="Expected incremental revenue",
)
...
st.caption(
    "The notebook’s fitted Hill function has slope fixed at 1; there is no learned slope parameter to expose. The half-saturation control is therefore the faithful teaching control."
)
