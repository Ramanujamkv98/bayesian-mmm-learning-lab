...
setup_page(
    ...
    "🎲",
    "Each posterior draw is one plausible combination of model parameters supported by the data and assumptions.",
)
...
result = historical_row_prediction(
    show, season, row, 200, 91, artifacts.fingerprint, artifacts
)
selected_id = st.session_state.world_draw
world = predict_scenario_row(
    scenario,
    row,
    artifacts,
    draw_indices=np.array([selected_id]),
    include_noise=False,
)
samples = result["samples"]
plot = pd.DataFrame(
    {"Posterior world": np.arange(len(samples)), "Predicted revenue": samples}
)
fig = px.scatter(
    plot,
    x="Posterior world",
    y="Predicted revenue",
    color_discrete_sequence=["rgba(91,95,239,.32)"],
    title="200 plausible posterior predictions",
)
fig.add_hline(
    y=np.median(samples),
    line_width=4,
...
m[3].metric("Its prediction", money(world["expected_samples"][0]))
    ...
    table = pd.DataFrame(
        {
            ...
            "Show response": artifacts.posterior["beta_show"][
                selected_id, artifacts.show_lookup[show]
            ],
        }
    )
    st.dataframe(
...
st.info(
    "MCMC is not fitting a separate model for every observation. It samples plausible shared parameter combinations, then each combination can predict the same scenario."
)
