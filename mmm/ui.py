...
def setup_page(title: str, icon: str, intro: str) -> None:
    st.set_page_config(
        page_title=f"{title} · MMM Learning Lab",
        page_icon=icon,
        layout="wide",
    )
    st.markdown(
        ...
        div[data-testid="stMetric"] {
          ...
          box-shadow:0 8px 30px rgba(45,47,105,.07);
        }
    ...
    st.markdown(
        '<div class="eyebrow">Bayesian MMM Learning Lab</div>',
        unsafe_allow_html=True,
    )
...
def scenario_picker(
    ...
    show = columns[0].selectbox(
        "Show",
        artifacts.coords["show"],
        key=f"{prefix}_show",
    )
    ...
    frame = data[
        (data["Show"] == show) & (data["Season"] == season)
    ].copy().reset_index(drop=True)
    week = columns[2].selectbox(
        "Lifecycle week",
        frame["Week_Number"].tolist(),
        key=f"{prefix}_week",
    )
